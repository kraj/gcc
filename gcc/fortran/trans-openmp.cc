			  ...
			  vec<tree> *new_iterators)
{
  ...
  gimple_seq *loops_seq_p = gimple_omp_target_iterator_loops_ptr (ctx);
  ...
  if (iterators)
    {
      ...
      assign_to_iterator_elems_array (data_expr, data_iter, ctx, false);
      data_expr = OMP_ITERATOR_ELEMS (data_iter);
  ...
  if (iterators)
    {
      ...
      assign_to_iterator_elems_array (data_addr_expr, data_addr_iter, ctx,
				      false);
      data_addr_expr = OMP_ITERATOR_ELEMS (data_addr_iter);
  ...
  if (iterators)
  {
    assign_to_iterator_elems_array (size_expr, data_iter, ctx, true);
  ...
  if (iterators)
  {
    assign_to_iterator_elems_array (bias_expr, data_addr_iter, ctx, true);
...
/* Map an array section or array element.
   ...
   CD identifies the clause and its context.
...
gfc_map_array_descriptor (
  ...
  location_t map_loc, stmtblock_t *block,
  toc_directive cd, gfc_omp_namelist *n,
				  ...
				  gfc_omp_namelist *clause, gfc_omp_udm *udm)
{
  ...
  gfc_omp_map_op outer_map_op = clause->u.map.op;
