...
lower_omp_map_iterator_expr (tree expr, tree c, gomp_target *stmt)
{
  ...
  assign_to_iterator_elems_array (expr, iterator, stmt, false);
...
lower_omp_map_iterator_size (tree size, tree c, gomp_target *stmt)
{
  ...
  tree iterator = OMP_CLAUSE_ITERATORS (c);
  assign_to_iterator_elems_array (size, iterator, stmt, true);
...
lower_omp_target (gimple_stmt_iterator *gsi_p, omp_context *ctx)
{
  ...
  if (ctx->record_type)
      ...
      else if (deep_map_cnt)
	{
	  ...
	  tree n = create_tmp_var_raw (size_type_node, "deep_map_cnt");
